  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between pb-1">
        <div>
          <p className="text-[10px] font-mono uppercase tracking-[0.12em] text-mirage-muted">Behavioral Finance</p>
          <h1 className="text-[22px] font-light text-white tracking-tight mt-0.5">Risk Overview</h1>
        </div>
        <div className="flex items-center gap-2">
          <div className={`flex items-center gap-1.5 text-[10px] font-mono px-2.5 py-1 border ${
            connected ? 'text-mirage-success border-mirage-success/30' : 'text-mirage-danger border-mirage-danger/30'
          }`}>
            <span className={`w-1 h-1 rounded-full ${connected ? 'bg-mirage-success' : 'bg-mirage-danger'}`} />
      ...
      {/* AI analysis banner */}
      {data?.ai_summary && (
        <div className="panel border-l-2 border-l-mirage-accent px-4 py-3">
          <p className="text-[10px] font-mono uppercase tracking-[0.08em] text-mirage-accent mb-1">AI Behavioral Analysis</p>
